-- Correção (2026-09-26): o caminho da foto do RQ03 usava o NOME da linha (ex.: "CHINÊS 8'"), e o Supabase
-- Storage recusa a letra acentuada "Ê" na chave do objeto ("Invalid key" — apóstrofo e espaço são aceitos,
-- letra acentuada não). Troca o nome pelo `id` (uuid) de pcp_laminadoras no caminho: nunca tem caractere
-- problemático e não muda mais se o nome da linha for editado no futuro. A coluna `linha` da tabela continua
-- guardando o nome (para filtro/exibição); só o caminho da foto muda.
