-- Bitola 2,0 sai do setup dos secadores: não existe item de lâmina seca com essa bitola no SAP
-- (bitolas do SAP: 1,5 / 1,8 / 2,2 / 2,5 / 2,7 / 3,1 / 3,3), então o apontamento nunca acharia item.
-- Nenhuma OP existente usa 2,0 (conferido antes). Única mudança em relação à versão anterior de
-- salvar_setup_secador: a lista de bitolas em v_ok.
...
begin
  ...
  -- Tipo/espécie/bitola/turno: também existem no portal (SETUP_OPCOES em pages/op/secagem.js) e no
  -- app (setup-secadores.js). Comprimento/largura vêm da tabela pcp_secagem_setup_medidas.
