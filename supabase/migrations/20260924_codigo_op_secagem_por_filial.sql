-- Código da OP de secagem alinhado ao padrão da Amarração: "SE" + filial + "-" + sequencial
-- crescente por filial (ex.: SE1-1, SE1-2), em vez de "SEC-<secador>-0001".
-- Renomeia o único registro existente (sem apontamentos) para não deixar um código fora do padrão.
update public.pcp_op_secagem set codigo_op = 'SE1-1' where codigo_op = 'SEC-FEZER-0001';

drop sequence if exists public.pcp_op_secagem_codigo_seq;
...
begin
  ...
  -- entre FEZER e OMECO, então os dois precisam serializar aqui para não colidir no número.
